...
impl super::TermWindow {
    ...
    fn start_tab_drag(&mut self, tab_idx: usize, start_event: MouseEvent) {
        log::debug!(
            "TAB_DRAG: start_tab_drag(idx={tab_idx}, coords=({},{}))",
    ...
    fn drag_tab_target_idx(&self, current_tab_idx: usize, cursor_x: isize) -> Option<usize> {
        let prev = current_tab_idx
            .checked_sub(1)
            .and_then(|i| self.tab_ui_item(i));
        let next = if self
            .last_tab_index()
            .map_or(false, |last| current_tab_idx < last)
        {
            self.tab_ui_item(current_tab_idx + 1)
        } else {
            None
        };
        compute_drag_target(current_tab_idx, cursor_x, prev.as_ref(), next.as_ref())
    }
    ...
    fn drag_tab(&mut self, event: &MouseEvent, context: &dyn WindowOps) -> bool {
        ...
        if event.mouse_buttons != WMB::LEFT {
            log::debug!("TAB_DRAG: drag_tab - not LEFT button, putting state back");
        ...
        if !state.has_dragged {
            log::debug!("TAB_DRAG: drag_tab - threshold exceeded, starting actual drag");
        }
        ...
        if let Some(target_idx) = target_idx {
            if target_idx != state.tab_idx {
                if let Err(err) = self.move_tab_by_idx(state.tab_idx, target_idx) {
...
/// Pure-function core of `drag_tab_target_idx`.
/// Given the current tab index, cursor x position, and optional neighbour UI items,
/// returns the target index to swap to (or None if cursor hasn't crossed a midpoint).
///
/// `prev` / `next` carry `(x, width)` of the neighbouring tab UI elements.
fn compute_drag_target(
    ...
    if let Some(prev_item) = prev {
        let prev_mid_x = prev_item.x as isize + prev_item.width as isize / 2;
    ...
    if let Some(next_item) = next {
        let next_mid_x = next_item.x as isize + next_item.width as isize / 2;
...
mod tests {
    ...
    // --- compute_drag_target pure-function tests ---

    use super::compute_drag_target;
    use crate::termwindow::{UIItem, UIItemType};

    fn tab_ui(x: usize, width: usize) -> UIItem {
        UIItem {
            x,
            y: 0,
            width,
    ...
    #[test]
    fn drag_stays_put_when_cursor_at_center() {
        // Tab 1 at x=0, w=100 → mid=50. Tab 2 at x=100, w=100 → mid=150.
        // Cursor at 100 (center of tab 2) → no swap.
        let prev = tab_ui(0, 100);
        let next = tab_ui(100, 100);
        assert_eq!(compute_drag_target(1, 100, Some(&prev), Some(&next)), None);
    }

    #[test]
    fn drag_swaps_left_past_prev_midpoint() {
        let prev = tab_ui(0, 100);
        let next = tab_ui(100, 100);
        // cursor at 49 < prev mid (50) → swap to idx 0
        assert_eq!(
            compute_drag_target(1, 49, Some(&prev), Some(&next)),
            Some(0)
        );
    }

    #[test]
    fn drag_swaps_right_past_next_midpoint() {
        let prev = tab_ui(0, 100);
        let next = tab_ui(100, 100);
        // cursor at 151 > next mid (150) → swap to idx 2
        assert_eq!(
            compute_drag_target(1, 151, Some(&prev), Some(&next)),
            Some(2)
        );
    }

    #[test]
    fn drag_no_swap_at_exact_prev_midpoint() {
        let prev = tab_ui(0, 100);
        assert_eq!(compute_drag_target(1, 50, Some(&prev), None), None);
    }

    #[test]
    fn drag_no_swap_at_exact_next_midpoint() {
        let next = tab_ui(100, 100);
        assert_eq!(compute_drag_target(0, 150, None, Some(&next)), None);
    }

    #[test]
    fn drag_first_tab_no_prev() {
        // First tab has no left neighbour; only check right.
        let next = tab_ui(100, 100);
        assert_eq!(compute_drag_target(0, 50, None, Some(&next)), None);
        assert_eq!(compute_drag_target(0, 151, None, Some(&next)), Some(1));
    }

    #[test]
    fn drag_last_tab_no_next() {
        // Last tab has no right neighbour; only check left.
        let prev = tab_ui(0, 100);
        assert_eq!(compute_drag_target(2, 100, Some(&prev), None), None);
        assert_eq!(compute_drag_target(2, 49, Some(&prev), None), Some(1));
    }

    #[test]
    fn drag_single_tab_no_neighbours() {
        assert_eq!(compute_drag_target(0, 0, None, None), None);
        assert_eq!(compute_drag_target(0, 999, None, None), None);
    }

    #[test]
    fn drag_unequal_tab_widths() {
        // Tab 0: x=0, w=200 → mid=100. Tab 2: x=200, w=80 → mid=240.
        let prev = tab_ui(0, 200);
        let next = tab_ui(200, 80);
        // cursor at 99 → left of prev mid → swap to 0
        assert_eq!(
            compute_drag_target(1, 99, Some(&prev), Some(&next)),
            Some(0)
        );
        // cursor at 100 → exact mid → no swap
        assert_eq!(compute_drag_target(1, 100, Some(&prev), Some(&next)), None);
        // cursor at 241 → past next mid → swap to 2
        assert_eq!(
            compute_drag_target(1, 241, Some(&prev), Some(&next)),
            Some(2)
        );
    }

    #[test]
    fn drag_many_tabs_rapid_sweep() {
        // Simulate 5 tabs of equal width 80, starting at x=0.
        // Tab positions: [0,80), [80,160), [160,240), [240,320), [320,400)
        // Mids: 40, 120, 200, 280, 360
        let tabs: Vec<UIItem> = (0..5).map(|i| tab_ui(i * 80, 80)).collect();

        // Drag tab 2 left: cursor must cross mid of tab 1 (120)
        assert_eq!(
            compute_drag_target(2, 119, Some(&tabs[1]), Some(&tabs[3])),
            Some(1)
        );
        // Drag tab 2 right: cursor must cross mid of tab 3 (280)
        assert_eq!(
            compute_drag_target(2, 281, Some(&tabs[1]), Some(&tabs[3])),
            Some(3)
        );
        // Tab 2 center range: [120, 280] → no swap
        assert_eq!(
