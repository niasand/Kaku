...
/// Lookup table for simple lazygit toast messages dispatched via EmitEvent.
        ...
        match notif {
            ...
            TermWindowNotif::MuxNotification(n) => match n {
                ...
                MuxNotification::PaneRemoved(pane_id) => {
                    ...
                    // so any overlay on a sibling needs its size
                ...
                MuxNotification::PaneAdded(_) => {
                    ...
                    // overlay open, the overlay must shrink too.
                    self.resize_overlays();
                    self.update_title_post_status();
                }
...
impl TermWindow {
    ...
    fn move_tab_by_idx(&mut self, from_idx: usize, tab_idx: usize) -> anyhow::Result<()> {
        let mux = Mux::get();
        let mut window = mux
            .get_window_mut(self.mux_window_id)
            .ok_or_else(|| anyhow!("no such window"))?;
        ...
        ensure!(from_idx < max, "cannot move a tab from out of range");
        ensure!(tab_idx < max, "cannot move a tab out of range");

        window
            .move_tab(from_idx, tab_idx)
            .ok_or_else(|| anyhow!("cannot move a tab out of range"))?;
    ...
    fn move_tab(&mut self, tab_idx: usize) -> anyhow::Result<()> {
        ...
        self.move_tab_by_idx(active, tab_idx)
    }
