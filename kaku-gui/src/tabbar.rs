...
fn tilde_path(path_str: &str) -> Option<String> {
...
fn format_path_segment(path_str: &str) -> String {
    if let Some(tilde) = tilde_path(path_str) {
        return tilde;
    }
...
fn tab_multi_pane_title(tab_id: TabId) -> Option<String> {
    ...
    for pos in panes.iter() {
        ...
        let segment = format_path_segment(path_str);
...
fn pane_cwd_title(pane: &PaneInformation) -> Option<String> {
    ...
    Some(format_path_segment(path_str))
}
...
fn build_default_title(
    ...
    // Tab shortcuts are appended by the tab bar renderer. Keep the classic
    // trailing space so the suffix has the same visual gap as Lua titles.
...
impl TabBarState {
    ...
    pub fn new(
        ...
        for tab_idx in 0..number_of_tabs {
            ...
            append_tab_shortcut_suffix(&mut tab_title, tab_idx, config);
            let mut esc =
...
/// Compute the 1-based (or 0-based if configured) display index for a tab.
pub fn tab_display_index(tab_idx: usize, config: &ConfigHandle) -> usize {
    tab_idx
...
fn title_text_ends_with_space(title: &TitleText) -> bool {
    title.items.iter().rev().find_map(|item| match item {
        FormatItem::Text(text) => text.chars().last().map(|ch| ch == ' '),
        _ => None,
    }) == Some(true)
}

fn append_tab_shortcut_suffix(title: &mut TitleText, tab_idx: usize, config: &ConfigHandle) {
    let Some(shortcut) = tab_shortcut_suffix(
        tab_idx,
        config.use_fancy_tab_bar,
    ...
    let spacing = tab_shortcut_spacing(title);
    title
        .items
        .push(FormatItem::Foreground(FormatColor::Default));
    title
        .items
        .push(FormatItem::Text(format!("{spacing}{shortcut}")));
}

fn tab_shortcut_spacing(title: &TitleText) -> &'static str {
    if title_text_ends_with_space(title) {
        " "
    } else {
        "  "
    }
...
fn tab_shortcut_suffix(
    tab_idx: usize,
    use_fancy_tab_bar: bool,
    show_tab_index_in_tab_bar: bool,
    zero_based_indices: bool,
) -> Option<String> {
    if use_fancy_tab_bar || !show_tab_index_in_tab_bar {
        return None;
    }

    let display_idx = tab_idx + if zero_based_indices { 0 } else { 1 };
    Some(format!("⌘+{display_idx}"))
}
...
mod test {
    ...
    #[test]
    fn home_path_titles_use_tilde_prefix() {
        let Some(home) = dirs_next::home_dir() else {
            return;
        };
        let path = home.join("Documents").join("Kaku");
        let Some(path) = path.to_str() else {
            return;
        };

        assert_eq!(format_path_segment(path), "~/Documents/Kaku");
    }

    #[test]
    fn shortcut_suffix_uses_command_key_index_for_classic_tabs() {
        assert_eq!(
            tab_shortcut_suffix(0, false, true, false).as_deref(),
            Some("⌘+1")
        );
        assert_eq!(
            tab_shortcut_suffix(0, false, true, true).as_deref(),
            Some("⌘+0")
        );
        assert!(tab_shortcut_suffix(0, true, true, false).is_none());
        assert!(tab_shortcut_suffix(0, false, false, false).is_none());
    }

    #[test]
    fn tab_title_suffix_preserves_two_cell_gap_after_padded_title() {
        let padded = TitleText {
            items: vec![FormatItem::Text("~/Documents/Kaku ".to_string())],
        };
        let unpadded = TitleText {
            items: vec![FormatItem::Text("~/Documents/Kaku".to_string())],
        };

        assert_eq!(tab_shortcut_spacing(&padded), " ");
        assert_eq!(tab_shortcut_spacing(&unpadded), "  ");
    }
