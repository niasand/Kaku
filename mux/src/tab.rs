...
fn pane_tree(
    ...
    match tree {
        ...
        Tree::Node { left, right, data } => {
            let data = data.expect("tree node must have data");
            PaneNode::Split {
                ...
                match data.direction {
                    SplitDirection::Vertical => {
                        ...
                        if x_adjust != 0 {
                            ...
                            data.first.cols = new_cols.try_into().expect("column count overflow");
                ...
                match data.direction {
                    SplitDirection::Horizontal => {
                        ...
                        if y_adjust != 0 {
                            ...
                            data.first.rows = new_rows.try_into().expect("row count overflow");
...
impl TabInner {
    ...
    fn toggle_pane_split_direction(&mut self) {
        ...
        loop {
            if cursor.is_leaf() {
                if cursor
                    .leaf_mut()
                    .expect("cursor must point to a leaf")
                    .pane_id()
                    == active_pane_id
                {
                    break;
    ...
    fn iter_panes_impl(&mut self, respect_zoom_state: bool) -> Vec<PositionedPane> {
        ...
        loop {
            if cursor.is_leaf() {
                ...
                let pane = Arc::clone(cursor.leaf_mut().expect("cursor must point to a leaf"));
    ...
    fn resize(&mut self, size: TerminalSize) {
        ...
        if let Some(zoomed) = &self.zoomed {
            ...
            let (min_x, min_y) =
                compute_min_size(self.pane.as_mut().expect("tab must have a pane"));
            ...
            apply_sizes_from_splits(self.pane.as_mut().expect("tab must have a pane"), &size);
        }
    ...
    fn split_and_insert(
...

        {
            ...
            let existing_pane = Arc::clone(cursor.leaf_mut().expect("cursor must point to a leaf"));
            ...
            *cursor.leaf_mut().expect("cursor must point to a leaf") = pane1;
