...
Edit `~/.config/kaku/assistant.toml` to enable Kaku Assistant, pick a provider, and enter your API key.
...
The overlay supports `/new`, `/resume`, `/clear`, `/status`, `/memory`, and `/exit`.
...
| `chat_model` | Deep Model for primary `Cmd + L` chat and tool use |
