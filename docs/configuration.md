...
Hidden when only one tab is open. Change position or shorten tab titles to the current directory name:

```lua
config.tab_bar_at_bottom = false        -- move to top
config.tab_title_show_basename_only = true  -- show "dirname" instead of "~/path/to/dirname"
```
