...
#   --notarize-only  Skip build; notarize existing dist/Kaku.app, upload.
#   --upload-only    Skip build + notarize; upload existing dist/Kaku.dmg.
#
...
main() {
    ...
    if [[ "$DRY_RUN" == "1" ]]; then
        ...
        log_warn "[DRY-RUN] Would build → notarize → tag V${version} → upload"
    ...
    if [[ "$SKIP_UPLOAD" -eq 0 ]]; then
        ...
        log_warn "Skipping upload (resume flag)"
    fi
