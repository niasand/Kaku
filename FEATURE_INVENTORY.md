...
| # | 功能 | 描述 | 类型 | 决策 |
|---|------|------|------|------|
| 1 | `kaku start` | 启动 GUI 终端，execvp 委托给 kaku-gui | Core | |
| 2 | `kaku` (无参数) | 交互式主菜单 TUI，键盘导航选子命令 | Extended | |
| 3 | `kaku config` | Lua 配置文件编辑 TUI（主题/字体/快捷键等 18 项设置） | Core | |
| 4 | `kaku init` | Shell 集成初始化（zsh/fish） | Core | |
| 5 | `kaku init --update-only` | 静默刷新集成文件，供 doctor 调用 | Core | |
| 6 | `kaku doctor` | 健康诊断（13 项检查），`--fix` 自动修复 | Core | |
| 7 | `kaku update` | 自更新（GitHub Release / Homebrew），SHA256 校验 | Extended | |
| 8 | `kaku reset` | 卸载清理（移除所有 Kaku 托管状态） | Extended | |
| 9 | `kaku ai` | AI 工具配置 TUI（9 个 provider） | AI | |
| 10 | `kaku chat` | exec 到 `k` 二进制，启动 AI 对话 | AI | |
| 11 | `kaku cli` | 实验性 mux server 交互（19 个子命令，隐藏） | Extended | |
| 12 | `kaku remote` | 显示 iOS 连接 QR 码（feature-gated） | Extended | |
| 13 | `kaku shell-completion` | 生成 shell 补全脚本 | Extended | |
| 14 | `kaku set-working-directory` | 发射 OSC 7 转义序列 | Core | |
| 15 | 全局 flag | `--skip-config`, `--config-file`, `--config k=v` | Core | |
...
| # | 功能 | 描述 | 类型 | 决策 |
|---|------|------|------|------|
| 16 | AI Chat Overlay (Cmd+L) | 全屏 AI 对话 TUI | AI | |
| 17 | SSE 流式输出 | 实时逐 token 显示 | AI | |
| 18 | Agent 循环 | 最多 25 轮工具调用 | AI | |
| 19 | 工具调用审批 | 危险操作需用户确认（Enter/Esc） | AI | |
| 20 | 对话持久化 | 保存/恢复对话，上限 100 条，LRU 淘汰 | AI | |
| 21 | Markdown 渲染 | AI 回复渲染为格式化 markdown | AI | |
| 22 | 语法高亮 | 代码块语法着色 | AI | |
| 23 | 多行输入 | Shift+Enter 换行 | AI | |
| 24 | IME 支持 | CJK 输入法合成文本支持 | AI | |
| 25 | Shift+Tab 切换模型 | 在可用模型间循环 | AI | |
| 26 | Chat/Fast 双模型 | 对话模型和后台任务用不同模型 | AI | |
| 27 | 微压缩 (micro-compact) | 截断过长工具输出，溢出到临时文件 | AI | |
| 28 | 记忆提取 | 每轮后用独立模型提取记忆写入 MEMORY.md | AI | |
| 29 | 对话标题生成 | 后台自动生成对话标题 | AI | |
| 30 | 终端上下文注入 | 最近 20 行终端输出 + 失败命令注入 prompt | AI | |
...
| 32 | `/resume` | 恢复历史对话 | AI | |
| 33 | `/clear` | 清空当前对话 | AI | |
| 34 | `/export` | 复制对话到剪贴板 | AI | |
| 35 | `/model` | 查看/切换模型 | AI | |
| 36 | `/config` | 显示当前 AI 配置 | AI | |
| 37 | `/memory` | 显示记忆文件路径 | AI | |
| 38 | `/status` | 显示会话状态 | AI | |
| 39 | `/btw` | 临时提问（不存历史） | AI | |
| 40 | `/check` | 代码审查（检查 diff 风险） | AI | |
| 41 | `/hunt` | 根因诊断 | AI | |
| 42 | `/think` | 功能规划 | AI | |
| 43 | `/read` | 网页/文档阅读 | AI | |
| 44 | `/write` | 文案润色 | AI | |
| 45 | `/learn` | 研究解释 | AI | |
| 46 | `/design` | UI 设计评审 | AI | |
| 47 | `/health` | AI 配置审计 | AI | |

### 2.3 AI 工具集

| # | 功能 | 描述 | 类型 | 决策 |
|---|------|------|------|------|
| 48 | `fs_read` | 读文件（可选行范围） | AI | |
| 49 | `fs_list` | 列目录 | AI | |
| 50 | `fs_write` | 写文件（自动创建父目录） | AI | |
| 51 | `fs_patch` | 查找替换文本 | AI | |
| 52 | `fs_mkdir` | 创建目录（递归） | AI | |
| 53 | `fs_delete` | 删除文件/目录 | AI | |
| 54 | `shell_exec` | 执行 shell 命令（60s 超时） | AI | |
| 55 | `shell_bg` / `shell_poll` | 后台进程 + 轮询 | AI | |
| 56 | `pwd` | 获取当前工作目录 | AI | |
| 57 | `grep_search` | 正则搜索（rg/grep fallback） | AI | |
| 58 | `symbol_search` | 语言感知的符号搜索 | AI | |
| 59 | `web_search` | 网页搜索（Brave/PipeLLM/Tavily） | AI | |
| 60 | `web_fetch` | 网页抓取为 Markdown | AI | |
| 61 | `http_request` | 通用 HTTP 客户端 | AI | |
| 62 | `read_url` | 优化的网页阅读（JS-heavy 支持） | AI | |
| 63 | `project_summary` | 项目摘要（语言/构建检测） | AI | |
| 64 | `file_tree` | 目录树（深度限制，过滤噪音） | AI | |
| 65 | `memory_read` | 读持久化记忆文件 | AI | |
| 66 | `soul_read` | 读用户身份文件 | AI | |
| 67 | 路径安全守卫 | 阻止访问 ~/.ssh、~/.aws 等敏感路径 | AI | |
| 68 | Shell 命令安全分析 | allowlist + 完整 shell parser 分类命令风险 | AI | |
| 69 | 输出预算系统 | 按 tool 分配字节上限，三级（brief/default/full） | AI | |

### 2.4 AI 对话附件

| # | 功能 | 描述 | 类型 | 决策 |
|---|------|------|------|------|
| 70 | `@cwd` | 附加当前目录结构 + git status + 关键文件预览 | AI | |
| 71 | `@tab` | 附加当前 pane 终端输出快照 | AI | |
| 72 | `@selection` | 附加当前选中文本 | AI | |

### 2.5 Soul / 身份系统

| # | 功能 | 描述 | 类型 | 决策 |
|---|------|------|------|------|
| 73 | SOUL.md | 用户身份文件（注入每个 system prompt，2KB 上限） | AI | |
| 74 | STYLE.md | 偏好回复风格 | AI | |
| 75 | SKILL.md | 工作模式描述 | AI | |
| 76 | MEMORY.md | 跨会话持久记忆（4KB 上限） | AI | |
| 77 | Onboarding Bootstrap | 首次使用时 AI 自动拆分回答生成身份文件 | AI | |

### 2.6 AI Provider 集成（`kaku ai` TUI 管理）

| # | 功能 | 描述 | 类型 | 决策 |
|---|------|------|------|------|
| 78 | Kaku Assistant | 内置 AI，API key + base URL + 模型选择 | AI | |
| 79 | Claude Code | 读 ~/.claude/settings.json，显示订阅/配额 | AI | |
| 80 | Codex | 读 ~/.codex/ 配置，显示推理等级/双窗口配额 | AI | |
| 81 | Kimi Code | OAuth 认证，显示模型/双窗口配额 | AI | |
| 82 | Antigravity (Codeium) | protobuf-over-HTTP 通信，配额窗口显示 | AI | |
| 83 | Gemini CLI | API key / OAuth / ADC 多认证，配额显示 | AI | |
| 84 | Copilot CLI | 月度配额 + 剩余次数显示 | AI | |
| 85 | Factory Droid | 模型 + 推理等级 + 自治等级 | AI | |
| 86 | OpenClaw | 认证 + 模型配置（含 legacy 路径 fallback） | AI | |
| 87 | Copilot/Codex OAuth | GitHub device-code OAuth + 60s 预过期刷新 | AI | |
...
| 88 | Cmd+N 新建窗口 | | Core | |
| 89 | Cmd+T 新建标签 | | Core | |
| 90 | Cmd+Shift+W 关闭标签 | | Core | |
| 91 | Cmd+W 关闭面板 | | Core | |
| 92 | Cmd+Shift+T 重开上次关闭的标签 | 恢复 cwd | Core | |
| 93 | Cmd+1..9 切换标签 | | Core | |
| 94 | Cmd+Shift+[ / ] 前后切换标签 | | Core | |
| 95 | Ctrl+Tab / Ctrl+Shift+Tab 切换标签 | | Core | |
| 96 | Cmd+Shift+O 标签导航器 | 交互式标签切换 | Core | |
| 97 | Ctrl+Shift+PageUp/Down 移动标签 | | Core | |
| 98 | Fancy Tab Bar | 富像素标签栏（图标/标题/关闭按钮） | Core | |
| 99 | Tab Bar 位置 | 顶部或底部（默认底部） | Core | |
| 100 | 标签索引显示 | 可配置显示/隐藏 | Extended | |
| 101 | 新建标签按钮 | 可配置显示/隐藏 | Extended | |
| 102 | 仅单标签时隐藏 Tab Bar | 可配置 | Extended | |
| 103 | Bell 指示器 | 未聚焦标签显示 bell 点 | Core | |
...
|---|------|------|------|------|
| 104 | Cmd+D 左右分割 | | Core | |
| 105 | Cmd+Shift+D 上下分割 | | Core | |
| 106 | Cmd+Alt+方向键 切换面板 | | Core | |
| 107 | Cmd+Ctrl+方向键 调整分割 | | Core | |
| 108 | Cmd+Shift+Enter 缩放面板 | 全屏切换 | Core | |
...
| 117 | Cmd+C 复制 | | Core | |
| 118 | Cmd+V 粘贴 | | Core | |
| 119 | 选中即复制 (copy_on_select) | 默认开启 | Core | |
| 120 | Cmd+K 清空 scrollback | | Core | |
...
|---|------|------|------|------|
| 124 | Cmd+F 搜索 | | Core | |
| 125 | Shift+PageUp/Down 翻页 | | Core | |
| 126 | Cmd+Home/End 滚到顶/底 | | Core | |
| 127 | 按语义提示符跳转 | ScrollToPrompt | Core | |
| 128 | 滚动条 | 可配置显示/隐藏 | Core | |
...
|---|------|------|------|------|
| 129 | Cmd+=/- 调整字体大小 | | Core | |
| 130 | Cmd+0 重置字体大小 | | Core | |
| 131 | Ctrl+Cmd+F 全屏切换 | | Core | |
| 132 | Cmd+Shift+Up 窗口置顶 | Always on Top | Extended | |
| 133 | Cmd+M 最小化 | | Core | |
| 134 | DPI 感知 | 按显示器 DPI 缩放 | Core | |
| 135 | 背景图片 | 可配置终端背景图 | Extended | |
| 136 | 窗口透明度 | window_background_opacity | Core | |
| 137 | 背景模糊 | macOS 原生 blur | Core | |
...
| # | 功能 | 描述 | 类型 | 决策 |
|---|------|------|------|------|
| 139 | Copy Mode (Ctrl+Shift+X) | Vim 风格键盘选择 + 搜索 | Core | |
| 140 | QuickSelect (Ctrl+Shift+Space) | 快速选择 URL/路径/Hash 等 | Core | |
| 141 | Command Palette (Cmd+Shift+P) | 命令面板 | Core | |
| 142 | Launcher | 命令/标签/工作区/域启动器 | Core | |
| 143 | Selector | Lua 定义的选择器 | Extended | |
| 144 | Character Select (Ctrl+Shift+U) | Emoji/Nerdfont 字符选择 | Extended | |
| 145 | Confirm / Prompt | 确认和输入对话框 | Extended | |
| 146 | ConfirmClosePane | 关闭面板确认 | Core | |
...
| # | 功能 | 描述 | 类型 | 决策 |
|---|------|------|------|------|
| 147 | 主题选择 | Auto (跟随 macOS) / Kaku Dark / Kaku Light | Core | |
| 148 | 1001 内置配色方案 | scheme_data.rs 中的大量预设 | Core | |
| 149 | 自定义配色方案 | 用户 TOML 文件 | Core | |
...
| 201 | 远程 AI 对话 | iOS 客户端通过 kaku-remote 发起 AI 对话 | AI | |
