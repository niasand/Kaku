...
mod imp {
    ...
    pub fn run(update_only: bool) -> anyhow::Result<()> {
        ...
        cleanup_k_wrapper().context("cleanup k wrapper")?;
...
"#
    ...
    /// Remove stale k wrapper left by older Kaku versions.
    /// The `k` binary was removed — this cleans up the orphaned wrapper script.
    fn cleanup_k_wrapper() -> anyhow::Result<()> {
        ...
        if k_path.exists() {
            ...
            if content.contains("Kaku") || content.contains("kaku") {
                fs::remove_file(&k_path)
                    .with_context(|| format!("remove stale k wrapper {}", k_path.display()))?;
                eprintln!("  Removed stale k wrapper (the `k` binary is no longer bundled).");
            }
        }
        Ok(())
    }
